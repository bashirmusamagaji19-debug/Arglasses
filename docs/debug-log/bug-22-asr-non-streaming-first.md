...
> 第一版我先抽象 ASR provider，保留 mock fallback，本地可切换 faster-whisper。这样项目已经具备“语音问题 -> 文本问题 -> 视觉记忆 pipeline”的能力，同时保留后续升级到实时 ASR 的清晰路径。后续默认切到 faster-whisper 后，又通过懒加载避免启动阶段直接加载模型。
