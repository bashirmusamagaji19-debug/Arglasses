...
> 当前推荐的第一视角输入页是 [浏览器原生 Live 输入页](live-input.md)：`/live`。本页记录的 `/mobile` 是拍照上传 fallback。

## 目标

`st.camera_input` 有时会受浏览器权限、HTTPS、内置浏览器和页面状态影响。为了让手机第一视角输入更稳定，早期新增了一个独立 FastAPI 手机页面：
...
手机只负责拍照上传，所有依赖仍然运行在本地电脑或云端后端。后续更实时的输入体验已经迁移到 `/live`：实时视频预览、录音转写、提问时截取当前帧。
